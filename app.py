...
lineaccesstoken = 'wylg0lP/vhCeXXIPTugMHVP8PY9O/+pEXzGOtHsmfkqDWCGSXIHcpWOusdzPvtu+2FdWA8Cm8g8+Wt+HoR+0YXCBJfAIOQDhH5HGVJrZeRqX+7kVh5f6IpJrSMGUMLUpuCuhSQTj2I2f1NylGSsBfwdB04t89/1O/w1cDnyilFU='
...
def event_handle(event,json_line):
    ...
    if msgType == "text":
        ...
        if (msg == "สวัสดี") :
            replyObj = TextSendMessage(text="ดีคั้บน้อง")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "ทำไรอยู่") :
            replyObj = TextSendMessage(text="ทำไรก็ได้โตแย้ว อิอิ")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "กินข้าวยัง") :
            replyObj = TextSendMessage(text="กินแล้วววววว")
            line_bot_api.reply_message(rtoken, replyObj)
        elif (msg == "มะยัง") :
            replyObj = TextSendMessage(text="ภันเต วิสุง วิสุง รักขะนัตถายะ")
            line_bot_api.reply_message(rtoken, replyObj)
           ...
           url = "https://dialogflow.cloud.google.com/v1/integrations/line/webhook/b1953abc-6bad-4fe0-b126-8d8e2e238da8"
